...
1. The BNO issues a *BillingState* to each of their Business Network members. The BNO can either pre-allocate an amount that a member can spent (bounded state), or leave the amount empty which would effectively allow the member to spent an unlimited number of *Billing Chips* (unbounded state, can be used for transaction metering). *Billing States* might include an *expiry date*, after which the states cen not be used anymore.
...
5. The BNO bills the members based on the reported *spent* amounts. After all obligations are settled, the BNO would unilaterally *close* all returned states. 
